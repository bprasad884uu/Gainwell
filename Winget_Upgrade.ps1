...
psFile.WriteLine "                `$headerLine = `$rawOutput | Where-Object { `$_ -match '\bId\b' } | Select-Object -First 1"
psFile.WriteLine "                `$idIndex = `$headerLine.IndexOf('Id')"
psFile.WriteLine "                `$upgradeList = `$rawOutput | Where-Object {"
psFile.WriteLine "                    `$_ -notmatch '^\s*$' -and"
psFile.WriteLine "                    `$_ -notmatch '^-+$' -and"
psFile.WriteLine "                    `$_ -notmatch '^Name' -and"
psFile.WriteLine "                    `$_ -notmatch 'upgrades available' -and"
psFile.WriteLine "                    `$_ -notmatch 'Copyright' -and"
psFile.WriteLine "                    `$_ -notmatch 'Windows Package'"
psFile.WriteLine "                }"
psFile.WriteLine "                `$packagesToUpgrade = foreach (`$line in `$upgradeList) {"
...
psFile.WriteLine "                    Write-Output ""Upgrading `$(`$packagesToUpgrade.Count) packages (Java excluded)..."""
...
psFile.WriteLine "                    Write-Output ""No packages to upgrade (or all were Java packages)."""
psFile.WriteLine "                }"
