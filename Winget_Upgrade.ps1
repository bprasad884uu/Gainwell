...
psFile.WriteLine "            Write-Output ""Winget found. Fetching upgradable packages..."""
psFile.WriteLine "            try {"
psFile.WriteLine "                `$rawOutput = & `$wingetExePath upgrade --accept-source-agreements 2>`$null"
psFile.WriteLine "                `$headerLineIndex = (`$rawOutput | Select-String '^Name\s+Id\s+').LineNumber - 1"
psFile.WriteLine "                if (`$headerLineIndex -ge 0) {"
psFile.WriteLine "                    `$headerLine = `$rawOutput[`$headerLineIndex]"
psFile.WriteLine "                    `$idIndex = `$headerLine.IndexOf('Id')"
psFile.WriteLine "                    `$dataLines = `$rawOutput | Select-Object -Skip (`$headerLineIndex + 2)"
psFile.WriteLine "                    `$packagesToUpgrade = foreach (`$line in `$dataLines) {"
psFile.WriteLine "                        if (`$line -match '^\s*$' -or `$line -match 'upgrades available') { continue }"
psFile.WriteLine "                        if (`$line.Length -gt `$idIndex) {"
psFile.WriteLine "                            `$rest = `$line.Substring(`$idIndex).Trim()"
psFile.WriteLine "                            `$packageId = (`$rest -split '\s+')[0]"
psFile.WriteLine "                            if (`$packageId -match '\.' -and `$packageId -notmatch 'java|jdk|jre|temurin|adoptium' -and `$packageId.Length -gt 3) {"
psFile.WriteLine "                                `$packageId"
psFile.WriteLine "                            }"
psFile.WriteLine "                        }"
psFile.WriteLine "                    }"
psFile.WriteLine "                    if (`$packagesToUpgrade) {"
psFile.WriteLine "                        Write-Output ''"
psFile.WriteLine "                        Write-Output ""Packages to be upgraded (Java excluded):"""
psFile.WriteLine "                        Write-Output `$rawOutput[`$headerLineIndex]"
psFile.WriteLine "                        Write-Output `$rawOutput[`$headerLineIndex + 1]"
psFile.WriteLine "                        foreach (`$line in `$dataLines) {"
psFile.WriteLine "                            if (`$line -match '^\s*$' -or `$line -match 'upgrades available') { continue }"
psFile.WriteLine "                            if (`$line.Length -gt `$idIndex) {"
psFile.WriteLine "                                `$rest = `$line.Substring(`$idIndex).Trim()"
psFile.WriteLine "                                `$packageId = (`$rest -split '\s+')[0]"
psFile.WriteLine "                                if (`$packageId -match '\.' -and `$packageId -notmatch 'java|jdk|jre|temurin|adoptium' -and `$packageId.Length -gt 3) {"
psFile.WriteLine "                                    Write-Output `$line"
psFile.WriteLine "                                }"
psFile.WriteLine "                            }"
psFile.WriteLine "                        }"
psFile.WriteLine "                        Write-Output ''"
psFile.WriteLine "                        Write-Output ""Upgrading `$(`$packagesToUpgrade.Count) packages..."""
psFile.WriteLine "                        foreach (`$pkg in `$packagesToUpgrade) {"
psFile.WriteLine "                            Write-Output ''"
psFile.WriteLine "                            Write-Output ""--- Upgrading: `$pkg ---"""
psFile.WriteLine "                            & `$wingetExePath upgrade --id `$pkg --accept-source-agreements --accept-package-agreements --silent"
psFile.WriteLine "                        }"
psFile.WriteLine "                        Write-Output ''"
psFile.WriteLine "                        Write-Output ""Upgrade completed successfully!"""
psFile.WriteLine "                    } else {"
psFile.WriteLine "                        Write-Output ""No packages to upgrade."""
psFile.WriteLine "                    }"
psFile.WriteLine "                } else {"
psFile.WriteLine "                    Write-Output ""Could not parse winget output."""
psFile.WriteLine "                }"
