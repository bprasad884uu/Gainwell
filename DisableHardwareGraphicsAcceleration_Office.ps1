...
foreach ($user in $Registry) {
    ...
    if ($username) {
        ...
        $ExcelRegistryPath = "Registry::HKEY_USERS\$user\Software\Microsoft\Office\16.0\Common\Graphics"
        ...
        Set-ItemProperty -Path $ExcelRegistryPath -Name "DisableHardwareAcceleration" -Value 1 -Type DWord
        Write-Host "Disabled Hardware Graphics Acceleration in Excel 16.0 for $username."
    } else {
