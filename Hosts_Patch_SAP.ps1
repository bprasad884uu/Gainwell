<#
.SYNOPSIS
  Replace Windows "hosts" and "services" files with provided content, with backup and restore support.

.DESCRIPTION
  - Creates backups at:
      C:\Windows\System32\drivers\etc\hosts.bak
      C:\Windows\System32\drivers\etc\services.bak
    (backups are only created once; existing .bak files are left alone)
  - Replaces the files with the embedded content (ASCII encoding).
  - Use -Restore to restore from the .bak files.
.EXAMPLE
  .\Replace-HostsAndServices.ps1           # replace (must be run as Admin)
  .\Replace-HostsAndServices.ps1 -Restore  # restore from backups
#>
...
$hostsBak    = Join-Path $etcLoc "hosts.bak"
$servicesPath= Join-Path $etcLoc "services"
$servicesBak = Join-Path $etcLoc "services.bak"

function Create-BackupIfMissing {
    param($SourcePath, $BackupPath)
    if (-not (Test-Path $SourcePath)) {
        Write-Warning "Source file not found: $SourcePath. Skipping backup."
        return
    }
    if (-not (Test-Path $BackupPath)) {
        try {
            ...
            Write-Error "Failed to create backup for $SourcePath -> $_"
            throw
        }
    } else {
...
function Restore-FromBackup {
    ...
    if (-not (Test-Path $BackupPath)) {
        Write-Warning "Backup not found: $BackupPath. Cannot restore $TargetPath."
        return
    }
    try {
        ...
        Write-Error "Restore failed for $TargetPath -> $_"
        throw
    }
}

if ($Restore) {
    Write-Host "Restore mode: restoring hosts and services from backups (if present)..."
    Restore-FromBackup -BackupPath $hostsBak -TargetPath $hostsPath
    Restore-FromBackup -BackupPath $servicesBak -TargetPath $servicesPath
    Write-Host "Restore completed."
    exit 0
}

# Create backups (only if missing)
Create-BackupIfMissing -SourcePath $hostsPath -BackupPath $hostsBak
Create-BackupIfMissing -SourcePath $servicesPath -BackupPath $servicesBak

# New hosts content (as provided)
$newHostsContent = @"
...
# New services content (as provided)
$newServicesContent = @"
...
# Write the new content (ASCII encoding)
try {
    Set-Content -Path $hostsPath -Value $newHostsContent -Force -Encoding ASCII
    Write-Host "Replaced hosts file: $hostsPath"
} catch {
...
try {
    Set-Content -Path $servicesPath -Value $newServicesContent -Force -Encoding ASCII
    Write-Host "Replaced services file: $servicesPath"
} catch {
    Write-Error "Failed to write services file: $_"
    throw
}

Write-Host "Operation completed. Backups are kept at:"
Write-Host " - $hostsBak"
Write-Host " - $servicesBak"
Write-Host "Use this script with -Restore to revert to the backups."
