...
$kbInstalled = Get-HotFix | Where-Object { $_.Description -like "*KB5034441*" }

if ($kbInstalled) {
    Write-Host "KB5034441 is already installed."
} else {
    Write-Host "KB5034441 is not installed. Taking additional actions..."
	Write-Host "-----------------------------"
	Write-Host "WinRE Partition Resizing..."
	Write-Host "-----------------------------"
...
LogMessage("OS partition Offset: " + $OSPartition.Offset)
LogMessage("")
LogMessage("Successfully completed WinRE repartition operation")
...
if ($searchResult.Updates.Count -eq 0) {
	...
	for ($i = 0; $i -lt $searchResult.Updates.Count; $i++) {
		...
		Write-Output ("{0}. {1}, Size: {2:N2} MB" -f ($i + 1), $update.Title, $size)
	...
	# Loop through each update to check installation result
	for ($i = 0; $i -lt $installationResult.Updates.Count; $i++) {
		# Output result
		if ($installationResult.GetUpdateResult($i).ResultCode -eq 2) {
			Write-Output ("Update {0} installed successfully." -f $updatesToDownload.Item($i).Title)
			$successfulUpdates++
		} else {
			Write-Output ("Failed to install update {0}." -f $updatesToDownload.Item($i).Title)
			$failedUpdates++
		}
	}

	Write-Output ("Installation finished. {0} updates installed successfully, {1} updates failed to install." -f $successfulUpdates, $failedUpdates)
}
