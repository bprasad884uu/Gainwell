...
function Format-Size {
    ...
    elseif ($abs -ge 1GB) { return ("{0:N2} GB" -f ($Bytes / 1GB)) }
    elseif ($abs -ge 1MB) { return ("{0:N2} MB" -f ($Bytes / 1MB)) }
...
function Disable-WindowsRecall {
    ...
    Set-RegValue -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsAI" -Name "DisableAIDataAnalysis" -Value 1 -Type "DWord"
    try {
...
function Set-LaptopHibernationDefaults {
    ...
    Set-RegValue -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\238C9FA8-0AAD-41ED-83F4-97BE242C8F20\7bc4a2f9-d8fc-4469-b07b-33eb785aaca0" -Name "Attributes" -Value 2 -Type "DWord"
    Set-RegValue -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\abfc2519-3608-4c2a-94ea-171b0ed546ab\94ac6d29-73ce-41a6-809f-6363ba21b47e" -Name "Attributes" -Value 2 -Type "DWord"
    try {
    ...
    @{
        ...
        Items = @(
            ...
            @{ Name = "Windows Update Download Cache"; Type = "Special"; Action = {
                    ...
                    try {
                        $result = Remove-CleanupTarget -Paths @("$env:WINDIR\SoftwareDistribution\Download\*")
                    } finally {
...
if ($shown) {
    foreach ($r in $shown) {
        ...
        Write-Host -NoNewline (" {0,12}" -f (Format-KB $r.Bytes)) -ForegroundColor Green
...
foreach ($d in $DriveReport) {
    ...
    if ($d.DiffRaw -lt 0) { $diffColor = [ConsoleColor]::Red }
    Write-Host -NoNewline ("  {0,-6}" -f $d.Drive) -ForegroundColor Yellow
    Write-Host -NoNewline (" {0,12}" -f $d.TotalStr) -ForegroundColor White
    Write-Host -NoNewline (" {0,12}" -f $d.UsedStr) -ForegroundColor Gray
    Write-Host -NoNewline (" {0,14}" -f $d.BeforeStr) -ForegroundColor Gray
    Write-Host -NoNewline (" {0,14}" -f $d.CleanedStr) -ForegroundColor Green
    Write-Host (" {0,12}" -f $d.DiffStr) -ForegroundColor $diffColor
}
