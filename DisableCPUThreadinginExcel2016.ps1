...
foreach ($user in $Registry) {
    ...
    if ($username) {
        # Disable Multi Threaded Calculation in Excel 16.0
        ...
        # Set MultiThreadedCalculation to 1
        Set-ItemProperty -Path $ExcelRegistryPath -Name "MultiThreadedCalculation" -Value 0 -Type DWord
        Write-Host "Disabled Multi Threaded Calculation in Excel 16.0 for $username."
    } else {
